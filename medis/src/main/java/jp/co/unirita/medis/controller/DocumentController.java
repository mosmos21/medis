...
public class DocumentController {
    ...
    public List<DocumentInfo> getDocumentsOfemployeeId(
            @AuthenticationPrincipal User user, @PathVariable(value = "employeeNumber") String employeeNumber,
            @PathVariable(value = "type", required = false) String publishType, @PathVariable(value = "size", required = false) Integer maxSize) throws InvalidArgumentException {
        ...
        if(publishType == null && maxSize == null) { // ユーザのすべてのドキュメントを取得する場合
            ...
            list = documentInfoRepository.findByEmployeeNumber(employeeNumber, new PageRequest(1, maxSize)).getContent();
        }
        return list;
    }
}
